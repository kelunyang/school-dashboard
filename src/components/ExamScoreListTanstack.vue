    ...
    <el-card>
      <template #header>
        <div class="card-header">
          <span>考試成績列表（{{ rawData.length }} 筆）</span>
          ...
          <tbody>
            ...
            >
              ...
              <td class="pinned-left">
                ...
                <el-checkbox
                  v-else
                  :model-value="isCrossFunctionalSelected(getStudentUID(item, 'examScore'))"
...
const handleCrossFunctionalChange = (student, checked) => {
  const uid = getStudentUID(student, 'examScore')
  if (!uid) return
  
  if (checked) {
    addToCrossFunctionalUIDs(uid, 'examScore')
  } else {
...
const handleDrop = (e, targetKey) => {
  ...
  if (draggedIndex !== -1 && targetIndex !== -1) {
    ...
    localStorage.setItem('examScoreListTanstackColumnOrder', JSON.stringify(columnOrder.value))
  }
}
...
const applyColumnSettings = () => {
  ...
  localStorage.setItem('examScoreListTanstackVisibleColumns', JSON.stringify(visibleColumnIds.value))
}
...
const loadData = async () => {
  ...
  try {
    ...
    if (lockedStudents.value && lockedStudents.value.length > 0) {
      result = await apiService.getCrossFunctionalExamScores(lockedStudents.value)
    } else {
      result = await apiService.getExamScores(props.selectedYear)
    }
    ...
    if (result.success) {
      ...
      ElMessage.error('載入考試成績失敗：' + result.error)
    }
  } catch (error) {
    ElMessage.error('載入考試成績失敗')
    console.error(error)
  } finally {
    loading.value = false
  }
...
const initializeColumns = () => {
  const savedVisible = localStorage.getItem('examScoreListTanstackVisibleColumns')
  if (savedVisible) {
    try {
  ...
  const savedOrder = localStorage.getItem('examScoreListTanstackColumnOrder')
  if (savedOrder) {
    try {
...
const generateReport = async () => {
  ...
  try {
    const filters = {
      ...
      reportType: 'examScore'
    }
    
    const result = await apiService.generateExamScoreReport(filters)
