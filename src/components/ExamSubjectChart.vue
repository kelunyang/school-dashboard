...
const drawChart = () => {
  ...
  props.scores.forEach(item => {
    if (item.score > 0 && item.score <= 15) { // 排除0分
  ...
  const scoreRange = Array.from({length: 15}, (_, i) => i + 1) // 1-15級分
  ...
  const getScoreColor = (score, year) => {
    ...
    if (scoreNum >= (benchmark.top || 15)) {
      return '#E74C3C' // 頂標：紅色
    } else if (scoreNum >= (benchmark.front || 12)) {
      return '#F39C12' // 前標：橙色
    } else if (scoreNum >= (benchmark.average || 10)) {
      return '#F1C40F' // 均標：黃色
    } else if (scoreNum >= (benchmark.back || 8)) {
      return '#95A5A6' // 後標：灰色
    } else if (scoreNum >= (benchmark.bottom || 6)) {
      return '#3498DB' // 底標：藍色
    } else {
...
const generateTableData = () => {
  ...
  // 創建15個級分的表格行
  const table = []
  for (let score = 1; score <= 15; score++) {
